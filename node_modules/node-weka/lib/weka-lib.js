...
//  });
  ...
  exports.classify2 = function (modelFile, trainFile, sentences, options, cb) {
      
      var fs = require('fs');


      var fileIdTraining;
      var workingFolder = options.workingDirectory;
      var predictionFile = workingFolder + '\\predict.arff';
      var filteredFile = workingFolder + '\\filtered.arff';
      var templateFile = workingFolder + '\\template.arff';

      console.log(templateFile);

      async.waterfall([
      // Clone template arff file to initialize prediction file
      function (callback) {
          var $in = fs.createReadStream(templateFile);
          var out = fs.createWriteStream(predictionFile);
          $in.pipe(out, { end: false });

          $in.on("end", function () {

              // Write the sentences to the arff file
              for (var i = 0, len = sentences.length; i < len; i++) {
                  out.write('"' + sentences[i] + '", Neutral\n'); // Neutral is constant because we don't know the real class yet
              }
      ...
      // Filter
      function (callback) {
          var command = 'java -classpath \"' + options.jarPath + '\" -Dfile.encoding=utf-8 ' + options.filter + ' ' + options.filterParams + '\" ' +
          '-b -i \"' + trainFile + '\" -o \"_temp.arff\" ' + // We have to call i and o so the dictionary will be built correctly
          '-r \"' + predictionFile + '\" -s \"' + filteredFile + '\" ';

          console.log("sentences:" + sentences); 
          console.log("options:" + options);
          console.log(cb);

          console.log("Filtering command:");
          console.log(command);
          child = exec(command,
            function (error, stdout, stderr) {
        ...
        function (callback) {
            ...
            var command = 'java -classpath "' + options.jarPath + '" -Dfile.encoding=utf-8 '
                  + options.classifier + ' ' +
                  ' -l "' + modelFile + '"' +
                  ' -T "' + filteredFile + '"' +
                  ' -no-cv -v -p 0 -c first ' +
            ...
            child = exec(command,
              function (error, stdout, stderr) {
                  ...
                  if (stderr) {
                      // Ignore discretizing warnings
                      if (stderr.indexOf("Warning: discretizing data set") === -1) {
                          console.log(stderr);
                          callback(stderr, {
                              classifier: options.classifier + ' ' + options.params
                          });
                          return;
                      }
                  }
                  ...
                  // First five rows and 2 last rows are redudant
                  for (var i = 5, len = allLines.length - 2; i < len; i++) {
                      var currLine = _.clean(allLines[i]),
                          splitted = currLine.split(' ');

                      if (isNaN(splitted[0]) || splitted[0] == "")
                          continue;

                      results.push({
                          predicted: splitted[2].split(':')[1],
                          certainty: splitted[splitted.length - 1]
                      });
                  }
  ...
  exports.classify4 = function (fid, trainFile, options, shouldFilter, cb) {
      var fs = require('fs');
      ...
      var filteredFile = workingFolder + '\\filtered.arff';
      var templateFile = workingFolder + '\\template.arff';
      var tempTrain = workingFolder + "\\_tempTrain" + fid.toString() + ".arff";
      ...
      function (callback) {
          ...
          var command = 'java -classpath \"' + options.jarPath + '\" -Dfile.encoding=utf-8 ' + options.filter + ' ' + options.filterParams + '\" ' +
          '-b -i \"' + trainFile + '\" -o \"' + tempTrain + '\" ' + // We have to call i and o so the dictionary will be built correctly
          '-r \"' + trainFile + '\" -s \"_tempTest.arff\" ';
            ...
            child = exec(command,
              function (error, stdout, stderr) {
                  ...
                  for (var i = 5, len = allLines.length - 2; i < len; i++) {
                      ...
                      results.push({
                          predicted: splitted[2].split(':')[1],
                          actual: splitted[1].split(':')[1],
                          certainty: splitted[splitted.length - 1]
                      });
                  }
                  callback(error, {
                      classifier: options.classifier + ' ' + options.params, results: results
                  });
              });
        }
      ], function (err, result) {
  ...
  exports.classify3 = function (trainFile, sentences, options, cb) {
